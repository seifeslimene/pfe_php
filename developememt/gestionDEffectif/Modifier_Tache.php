...
if(isset($_GET['id_tache']))
{
 if(!empty($_GET['id_tache']))
 {
   $id_tache   = $_GET['id_tache'];
      $req="select p.* from `tache`as t join `projet` as p where p.`ID`= t.`REFPROJET` AND t.ID=".$id_tache;
   $res=$conn->query($req);
   $data=array();
  ...
  if(isset($_POST))
  {
	if(!empty($_POST))
	{
		...
		if(sizeof($e)<1)
		{
		$req_mo  = "UPDATE  `tache` set Nom='".$nom."' where ID =".$id_tache;	
		$conn->query($req_mo);
		header('location:Affichage_Taches.php?id_projet='.$k);
		}
		}
		}
		 }
...
<head>
  <meta charset="UTF-8" />
	...
	<title>Modifier Une Client</title>
...
<div class="menubar">
	...
	<div class="page-title">
			<strong>Modification D'une T&acirc;che Du Projet <font color="red"> "<?php echo $n; ?>"</font></strong>
	</div>
</div>

<div class="content-wrapper">
<div class="content-wrapper"><a href="Liste_Taches.php?id_tache=<?php echo $id_tache ?>" class="btn btn-primary">Liste Des T&acirc;ches</a></div>
	...
	<form id="new-customer" class="form-horizontal" method="post" action="" role="form">
	  	...
	  	<div class="form-group form-actions">
	    	<div class="col-sm-offset-2 col-sm-10">
	    		<a href="Affichage_Taches.php?id_projet=<?php echo ($k); ?>" class="btn btn-default">Annuler</a>

...
if(isset($_GET['id_client']))
{
 if(!empty($_GET['id_client']))
 {
  ...
  $req1 = "select p.* from `personne` as p join `client` as c where c.REFPERSONNE=p.ID AND p.ID=".$id_client;
...
<head>
  <meta charset="UTF-8" />
	...
	<title><?php   foreach($data1 as $d) {
		  echo "Bienvenue ".$d['Nom']." ".$d['Prenom'];
		  var_dump($d['Nom']);}?></title>
...
<a href="Profil_Emp.php" class="sidebar">
      ...
      <li class="option">
        <a href="Projet_Client.php?id_client=<?php  foreach($data1 as $d) { echo $d['ID']; } ?>" data-toggle="sidebar">
    ...
    <ul>
	<li class="option">
<a href="Espace_Client.php?id_client=<?php   foreach($data1 as $d) { echo $d['ID'];} ?>" class="sidebar">
...
<div class="menubar">
	...
	<div class="page-title">
			<strong><h4 style="margin-top:0px;"><?php   foreach($data1 as $d) {  ?><font size="5"><?php echo "Bienvenue "?></font> <font color="green" face="monotype corsive" size="5"> <?php echo ' '.$d['Nom']." ".$d['Prenom'];?> <?php echo ' ' ?></font><?php
		  }?></H4></strong>
	</div>
</div>
...
<div class="content-wrapper">
	...
	<table  class="table">
		...
		<?php foreach($data as $d2) {?>
            <tr>
                <td><?php echo $d2['Nom_Proj'];?></td>
				<td><i><?php echo $d2['Description_Proj'];?></i></td>
            </tr>
         <?php }?>
